...
const value = (PageId, PageNumber) => {
        ...
        if (PageId == PageNumber + 1) {
            return divTracker[PageNumber - 1]
        }
...
export const NavfunctionTop = (PageNumber) => {
    ...
    for (var i = 0; i <TextTips.length-2; i++) {
        PagebarTop.push(<Pagebarfunction number={i+2} link={`${PageLinks[i+1]}`}
        current={PageNumber == i+2} 
        enabled={ProgressArray[0] >= divTracker[i] * parseInt(parseFloat(unitProgress[0]))} 
        text={`${TextTips[i+1]}`} />);
      }
...
export const NavfunctionBottom = (PageNumber) => {
    ...
    for (var i = 0; i <TextTips.length-2; i++) {
        ...
        enabled={ProgressArray[0] >= divTracker[i] * parseInt(parseFloat(unitProgress[0])) || i+1==PageNumber}  
        text={`${TextTips[i+1]}`} />);
      }
